...
// 	// 确保 ModeFSM 已构建，此时安全注册自定义移动模式
// 	AddMovementModeFromClass(TEXT("LrWalk"), ULrWalkMovementMode::StaticClass());
 ...
 * @param InMoveInputType 
 * @param InMoveInput 
 */
void FLrDefaultInputs::SetMoveInput(ELrMoveInputType InMoveInputType, const FVector& InMoveInput)
{
	...
	// 直接截断到 2 位小数，和 NetSerialize 里的量化精度保持一致，避免浮点误差导致预测回滚
...
// 如果站在移动基座上，把“本地方向”转成世界方向；否则原值就是世界方向
FVector FLrDefaultInputs::GetMoveInput_WorldSpace() const
{
...
FVector FLrDefaultInputs::GetOrientationIntentDir_WorldSpace() const
{
	if (bUsingMovementBase && MovementBase)
	{
...
FMoverDataStructBase* FLrDefaultInputs::Clone() const
{
	// TODO: ensure that this memory allocation jives with deletion method
	FLrDefaultInputs* CopyPtr = new FLrDefaultInputs(*this);
	return CopyPtr;
}

// 网络序列化：手动位压缩，带宽省到极限
bool FLrDefaultInputs::NetSerialize(FArchive& Ar, UPackageMap* Map, bool& bOutSuccess)
{
	Super::NetSerialize(Ar, Map, bOutSuccess);

	Ar << MoveInputType;
	// 向量量化：±100 m 范围，3 cm 精度，30 bit
	...
	Ar.SerializeBits(&bUsingMovementBase, 1); // 1 bit

	if (bUsingMovementBase)
	{
		Ar << MovementBase; // 对象指针网络序列化
		Ar << MovementBaseBoneName;
	}
	else if (Ar.IsLoading()) // 读包时把基座清掉，防止野指针
	{
...
// 调试字符串：一行行打印，方便控制台排查
void FLrDefaultInputs::ToString(FAnsiStringBuilderBase& Out) const
{
	Super::ToString(Out);
...
// 预测回滚核心：本地与服务器不一致就回滚
bool FLrDefaultInputs::ShouldReconcile(const FMoverDataStructBase& AuthorityState) const
{
	const FLrDefaultInputs& TypedAuthority = static_cast<const FLrDefaultInputs&>(AuthorityState);
	return *this != TypedAuthority;
}

// 网络丢包时补帧：线性插值输入，避免瞬移
void FLrDefaultInputs::Interpolate(const FMoverDataStructBase& From, const FMoverDataStructBase& To, float Pct)
{
	const FLrDefaultInputs& TypedFrom = static_cast<const FLrDefaultInputs&>(From);
	const FLrDefaultInputs& TypedTo = static_cast<const FLrDefaultInputs&>(To);

	// 取最近一帧的“一次性”状态
	// Note, this ignores movement base as this is not used by the physics mover
	const FLrDefaultInputs* ClosestInputs = Pct < 0.5f ? &TypedFrom : &TypedTo;
...
// 多输入源合并：跳只要有一帧按下就保留
void FLrDefaultInputs::Merge(const FMoverDataStructBase& From)
{
	const FLrDefaultInputs& TypedFrom = static_cast<const FLrDefaultInputs&>(From);
...
void FLrDefaultInputs::Decay(float DecayAmount)
{
...
FMoverDataStructBase* FLrMoverDefaultSyncState::Clone() const
{
	// TODO: ensure that this memory allocation jives with deletion method
	FLrMoverDefaultSyncState* CopyPtr = new FLrMoverDefaultSyncState(*this);
	return CopyPtr;
}

bool FLrMoverDefaultSyncState::NetSerialize(FArchive& Ar, UPackageMap* Map, bool& bOutSuccess)
{
	Super::NetSerialize(Ar, Map, bOutSuccess);

	SerializePackedVector<100, 30>(Location, Ar);
	SerializeFixedVector<2, 8>(MoveDirectionIntent, Ar); // 意图方向仅 8 bit
	SerializePackedVector<10, 16>(Velocity, Ar); // 速度 ±10 km/s，16 bit
...
// 调试打印
void FLrMoverDefaultSyncState::ToString(FAnsiStringBuilderBase& Out) const
{
	Super::ToString(Out);
...
// 权威对比：位置误差 > 5 cm 或基座不同就回滚
bool FLrMoverDefaultSyncState::ShouldReconcile(const FMoverDataStructBase& AuthorityState) const
{
	const FLrMoverDefaultSyncState* AuthoritySyncState = static_cast<const FLrMoverDefaultSyncState*>(&AuthorityState);
...
// 插值补帧：跨基座时先把 From 转到 To 的基座空间再 Lerp，避免电梯瞬移
void FLrMoverDefaultSyncState::Interpolate(const FMoverDataStructBase& From, const FMoverDataStructBase& To, float Pct)
{
	const FLrMoverDefaultSyncState* FromState = static_cast<const FLrMoverDefaultSyncState*>(&From);
	const FLrMoverDefaultSyncState* ToState = static_cast<const FLrMoverDefaultSyncState*>(&To);
	...
	else
	{
		...
		else if (ToState->MovementBase.IsValid()) // if moving onto a different base, regardless of coming from a prior base or not  // 上到新基座：把 From 的世界量转本地
		{
		...
		else if (FromState->MovementBase.IsValid()) // if leaving a base // 离开基座：直接用世界量
		{
...
// 把世界空间量写进来，内部自动转本地空间
void FLrMoverDefaultSyncState::SetTransforms_WorldSpace(FVector WorldLocation, FRotator WorldOrient, FVector WorldVelocity, UPrimitiveComponent* Base, FName BaseBone)
{
	SetTransforms_WorldSpace(WorldLocation, WorldOrient, WorldVelocity, FVector::ZeroVector, Base, BaseBone);
}

void FLrMoverDefaultSyncState::SetTransforms_WorldSpace(FVector WorldLocation, FRotator WorldOrient, FVector WorldVelocity, FVector WorldAngularVelocityDegrees, UPrimitiveComponent* Base, FName BaseBone)
{
...
// 设置基座并立即捕获其当前变换
bool FLrMoverDefaultSyncState::SetMovementBase(UPrimitiveComponent* Base, FName BaseBone)
{
	MovementBase = Base;
...
// 重新捕获基座当前变换，保持相对量不变
bool FLrMoverDefaultSyncState::UpdateCurrentMovementBase()
{
...
// 重新捕获基座当前变换，保持相对量不变
bool FLrMoverDefaultSyncState::IsNearlyEqual(const FLrMoverDefaultSyncState& Other) const
{
...
// 近似相等比较：位置/旋转/速度 + 基座信息都要对上
FVector FLrMoverDefaultSyncState::GetLocation_WorldSpace() const
{
	if (MovementBase.IsValid())
	{
...
FVector FLrMoverDefaultSyncState::GetLocation_BaseSpace() const
{
	return Location;
}


FVector FLrMoverDefaultSyncState::GetIntent_WorldSpace() const
{
	if (MovementBase.IsValid())
	{
...
FVector FLrMoverDefaultSyncState::GetIntent_BaseSpace() const
{
	return MoveDirectionIntent;
}

FVector FLrMoverDefaultSyncState::GetVelocity_WorldSpace() const
{
	if (MovementBase.IsValid())
	{
...
FVector FLrMoverDefaultSyncState::GetVelocity_BaseSpace() const
{
	return Velocity;
}


FRotator FLrMoverDefaultSyncState::GetOrientation_WorldSpace() const
{
	if (MovementBase.IsValid())
	{
...
FRotator FLrMoverDefaultSyncState::GetOrientation_BaseSpace() const
{
	return Orientation;
}

FTransform FLrMoverDefaultSyncState::GetTransform_WorldSpace() const
{
	if (MovementBase.IsValid())
	{
...
FTransform FLrMoverDefaultSyncState::GetTransform_BaseSpace() const
{
	return FTransform(Orientation, Location);
}

FVector FLrMoverDefaultSyncState::GetAngularVelocityDegrees_WorldSpace() const
{
	if (MovementBase.IsValid())
	{
...
FVector FLrMoverDefaultSyncState::GetAngularVelocityDegrees_BaseSpace() const
{
...
// void UMoverDataModelBlueprintLibrary::SetDirectionalInput(FLrDefaultInputs& Inputs, const FVector& DirectionInput)
// {
// 	Inputs.SetMoveInput(EMoveInputType::DirectionalIntent, DirectionInput);
// }
//
// void UMoverDataModelBlueprintLibrary::SetVelocityInput(FLrDefaultInputs& Inputs, const FVector& VelocityInput)
// {
// 	Inputs.SetMoveInput(EMoveInputType::Velocity, VelocityInput);
// }
//
// FVector UMoverDataModelBlueprintLibrary::GetMoveDirectionIntentFromInputs(const FLrDefaultInputs& Inputs)
// {
// 	return Inputs.GetMoveInput_WorldSpace();
// }
//
// FVector UMoverDataModelBlueprintLibrary::GetLocationFromSyncState(const FLrMoverDefaultSyncState& SyncState)
// {
// 	return SyncState.GetLocation_WorldSpace();
// }
//
// FVector UMoverDataModelBlueprintLibrary::GetMoveDirectionIntentFromSyncState(const FLrMoverDefaultSyncState& SyncState)
// {
// 	return SyncState.GetIntent_WorldSpace();
// }
//
// FVector UMoverDataModelBlueprintLibrary::GetVelocityFromSyncState(const FLrMoverDefaultSyncState& SyncState)
// {
// 	return SyncState.GetVelocity_WorldSpace();
// }
//
// FVector UMoverDataModelBlueprintLibrary::GetAngularVelocityDegreesFromSyncState(const FLrMoverDefaultSyncState& SyncState)
// {
// 	return SyncState.GetAngularVelocityDegrees_WorldSpace();
// }
//
// FRotator UMoverDataModelBlueprintLibrary::GetOrientationFromSyncState(const FLrMoverDefaultSyncState& SyncState)
// {
